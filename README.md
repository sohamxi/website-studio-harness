...
**Post-ship: client intake, autonomous feedback, and cost telemetry.** Three additions, all optional and additive:
- `skills/website-studio/references/client-intake-template.md` — a ~10-minute client-facing form, read by Phase 0 before it starts inferring. Its most important section (reference-anchor sites with a client-assigned 1-10 score) is the single field whose absence caused this harness's largest documented miscalibration (see `skills/studio-evolve/` — Run 1 scored a sparse-editorial site 8.5 internally against a 7.0 real-user score, entirely because nobody had asked the client what "good" looks like to them). Save the filled form to `_workspace/00_input/client-intake.md` before a run; Phase 0 picks it up automatically.
- `website-studio feedback --dir <path>` — a bounded (max 2 rounds), autonomous post-ship client-feedback loop: generates a short non-technical feedback form, waits for a real client response saved to `_workspace/13_client_feedback/response-round-N.md`, converts it into an owner-keyed fix list, dispatches fixes, and re-ships. A verdict of `STRUCTURAL` (the client's issue is with the direction, not the execution) stops the loop for a manual Phase 3 pass rather than mechanically patching around a wrong concept — same discipline as the gauntlet loop. See `skills/client-feedback-loop/`.
- `website-studio report --dir <path>` — per-phase wall-clock and (when the driver reports it) cost/token telemetry, recorded automatically to `_workspace/00_input/run-telemetry.jsonl` on every driver call. The `claude` driver captures real cost via `--output-format json`; other drivers report wall-clock only until they expose their own cost data — the field is left `null` rather than estimated.
